...
class SEBlock(nn.Module):
    """
    Squeeze-and-Excitation Block: 让网络拥有“全局视野”和“记忆”
    """
    def __init__(self, in_channels, reduction=4):
        super().__init__()
        self.avg_pool = nn.AdaptiveAvgPool2d(1) # 全局平均池化，压缩成 1x1
...
class ResidualConvBlock(nn.Module):
    def __init__(self, n_state: int, kernel_size: int = 31):
        super().__init__()
        
        # 时间混合层，使用 Pre-Activation 结构: BN -> SiLU -> Conv
        self.time_bn = nn.BatchNorm2d(n_state)
        ...
        # 通道混合层
        self.channel_bn = nn.BatchNorm2d(n_state)
        # 4倍宽度的 MLP，用 1x1 卷积实现
        n_mlp = n_state * 4
...
class AudioEncoder(nn.Module):
    def __init__(self, n_mels: int, n_state: int, n_layer: int):
        super().__init__()
        # 下采样和特征提取
        ...
        self.blocks = nn.ModuleList(
            [ResidualConvBlock(n_state) for _ in range(n_layer)]
        )
        
        self.bn_post = nn.BatchNorm2d(n_state)

    def forward(self, x: Tensor) -> Tensor:
        # x: B x 80 x 3000 x 1
        x = F.silu(self.conv1(x)) # -> B x n_state x 3000 x 1
        x = F.silu(self.conv2(x)) # -> B x n_state x 1500 x 1

        for block in self.blocks:
            x = block(x)

        return self.bn_post(x)
